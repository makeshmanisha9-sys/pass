...
const PropertyDetail = {
      ...
      <div style="display: grid; grid-template-columns: 1.4fr 1fr; gap: 28px;">
        ...
        <div>
          ...
          <div style="display: flex; flex-direction: column; gap: 16px; margin-bottom: 28px;">
            ${this.reviews.length === 0 ? '<p style="color:var(--text-muted)">No reviews yet for this listing.</p>' : 
              this.reviews.map(r => `
            ...
            <div style="display: flex; flex-direction: column; gap: 14px; margin-bottom: 20px;">
              ...
              <div class="form-group">
                ...
                <select id="bookGuests">
                  ${Array.from({length: p.maxGuests}, (_, i) => `<option value="${i+1}">${i+1} Guest${i > 0 ? 's' : ''}</option>`).join('')}
                </select>
              </div>
            </div>
  ...
  initMap() {
    ...
    if (typeof L !== 'undefined') {
      ...
      L.marker([p.location.lat, p.location.lng])
        .addTo(this.mapInstance)
        .bindPopup(`<b>${p.title}</b><br>${p.neighborhood}, ${p.city}`)
        .openPopup();
    } else {
  ...
  async submitReview(e) {
    ...
    try {
      ...
      await App.apiRequest(`/reviews/property/${this.currentProperty._id}`, 'POST', { rating, comment });
